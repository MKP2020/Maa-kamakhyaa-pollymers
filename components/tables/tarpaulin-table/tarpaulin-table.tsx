...
import { TTarpaulinFull } from "@/lib/schemas";
import { getTarpaulinList } from "@/actions/tarpaulin";
import { generatePdf } from "@/lib/generate-pdf/tarpaulin";

interface DataTableProps {
  columns: ColumnDef<TTarpaulinFull, any>[];
  data: TTarpaulinFull[];
  searchKey: string;
  pageNo: number;
  total: number;
...
export function TarpaulinTable({
  columns,
  data,
  pageNo,
  searchKey,
      ...
      <div id="table-indent" className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-4">
          <Button
            ...
            onClick={async () => {
              setDownloading(true);
              const { data } = await getTarpaulinList(
                shift,
                pFrom || undefined,
                pTo || undefined
              );

              await generatePdf(data, `tarpaulin-list`);
          ...
          <Button
            className="text-xs md:text-sm"
            onClick={() => router.push(`/dashboard/tarpaulin/new`)}
          >
