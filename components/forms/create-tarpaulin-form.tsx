...
const formSchema = z.object({
  ...
  tarpaulinGrade: z.string().regex(/^\d+\.?\d*$/, "Please select a grade"),
  tarpaulinSize: z.string(),
  tarpaulinQty: z.string().regex(/^\d+\.?\d*$/),

  tarpWaste: z.string().regex(/^\d+\.?\d*$/),
...
type TCreateTarpaulinFormValues = z.infer<typeof formSchema>;

type TUnitFormItemProps = {
  form: UseFormReturn<TCreateTarpaulinFormValues>;
  departments: TDepartment[];
  categories: TCategory[];
  index: number;
  disabled: boolean;
  onPressRemove: () => void;
  initial?: TTarpaulinItemFull;
  isLast: boolean;
};
...
export const CreateTarpaulinForm: FC<
  Pick<TUnitFormItemProps, "categories" | "departments"> & {
    initialData?: TTarpaulinFull;
    laminatedFabricGrades: TGradeWithQuantity[];
    tarpaulinGrades: TGrade[];
  }
> = (props) => {
  const {
    categories,
  ...
  const title = !!initialData ? "View Tarpaulin" : "Create Tarpaulin";
  const description = !!initialData
    ? "view tarpaulin"
    : "Create a new tarpaulin";

  const [loading, setLoading] = useState(false);

  const action = initialData ? "Update" : "Add";

  const defaultValues: TCreateTarpaulinFormValues = !!initialData
    ? {
        ...
        tarpWaste: initialData.tarpWaste.toString(),
        tarpaulinGrade: initialData.tarpaulinGradeId.toString(),
        tarpaulinQty: initialData.tarpaulinQty.toString(),
        tarpaulinSize: initialData.tarpaulinSize.toString(),
  ...
  const form = useForm<TCreateTarpaulinFormValues>({
  ...
  const onSubmit = useCallback(async (data: TCreateTarpaulinFormValues) => {
    const {
      shift,
      lamFabricGrade,
      lamFabricQty,
    ...
    try {
      setLoading(true);
      // TODO: create
      await createTarpaulin(
        {
          shift,
          lamFabricGradeId: Number(lamFabricGrade),
          lamFabricQty: Number(lamFabricQty),
          tarpaulinGradeId: Number(tarpaulinGrade),
          tarpaulinQty: Number(tarpaulinQty),
          tarpaulinSize: tarpaulinSize,
          tarpWaste: tarpWaste?.length === 0 ? 0 : Number(tarpWaste),
          date,
        },
        items.map(
      ...
      router.push(`/dashboard/tarpaulin`);
      router.refresh();
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  const lamFabricGradeType = form.watch("lamFabricGrade");

  const lamFabricGrade = laminatedFabricGrades.find((item) =>
    !lamFabricGradeType ? false : item.id === Number(lamFabricGradeType)
  );
  // useEffect(() => {
  //   (async () => {
  //     try {
    ...
    <>
      ...
      <Form {...form}>
        ...
        >
          ...
          <Separator className="my-2" />
          <h3>Laminated Fabric Consumed</h3>
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    ...
                    {!!lamFabricGrade
                      ? `(Available ${
                          (lamFabricGrade.quantity?.producedQty || 0) -
                          (lamFabricGrade.quantity?.usedQty || 0)
                        })`
                      : ""}
                  </FormLabel>
                  <FormControl inputMode="numeric">
          ...
          <div className="md:grid md:grid-cols-2 lg:grid-cols-4 gap-8 space-y-8 lg:space-y-0 items-center">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel>Tarpaulin Grade</FormLabel>
                  <Select
                    disabled={isDisabled}
                    onValueChange={field.onChange}
                    value={field.value}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Tarpaulin Size</FormLabel>
                  <FormControl inputMode="numeric">
                    <Input
                      disabled={isDisabled}
                      placeholder="Enter quantity"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Tarpaulin Quantity</FormLabel>
                  <FormControl inputMode="numeric">
                    <Input
                      disabled={isDisabled}
                      placeholder="Enter quantity"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Tarp Waste</FormLabel>
                  <FormControl inputMode="numeric">
                    <Input
                      min={0}
                      disabled={isDisabled}
